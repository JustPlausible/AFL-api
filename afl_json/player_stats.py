...
from .client import AflJsonClient, AflJsonError, AflJsonInvalidResponse, AflJsonResourceUnavailable
...
class MatchStatusResolution:
    """Status metadata resolved independently from the player-stat payload."""
    ...
    canonical_refreshed: bool = False
...
def normalise_player_stats(payload: Any, match_provider_id: str, *, collected_at: str,
    ...
    if (endpoint_source_status and resolved_match_status
        ...
        diagnostics.append(PlayerStatDiagnostic(
            ...
            f"Endpoint status {endpoint_source_status!r} conflicts with canonical match status "
            f"{resolved_match_status!r} for match {match_provider_id}; the forward-most "
            "recognised lifecycle status is used",
        ))
    effective_status = _forward_status(endpoint_source_status, resolved_match_status)
    records: list[CanonicalPlayerStat] = []
    rejected = 0
    seen: dict[str, str] = {}
...
def resolve_authoritative_match_status(conn: sqlite3.Connection, client: AflJsonClient, *,
                                       match_provider_id: str,
                                       afl_match_id: int | None = None) -> MatchStatusResolution:
    """Resolve stored metadata, then refresh it from direct match detail when useful.

    Round/list match responses are useful for bulk collection but have been
    observed lagging the direct ``/matches/{id}`` resource during finalisation.
    A stored concluded status is terminal and therefore needs no network check.
    """
    row = conn.execute(
        "SELECT match_id, status FROM matches WHERE match_provider_id=?"
        + (" OR match_id=?" if afl_match_id is not None else "")
        + " ORDER BY CASE WHEN match_provider_id=? THEN 0 ELSE 1 END LIMIT 1",
        ((match_provider_id, afl_match_id, match_provider_id) if afl_match_id is not None
         else (match_provider_id, match_provider_id)),
    ).fetchone()
    stored_match_id = _integer_id(row[0]) if row else None
    stored_status = _text(row[1]) if row else None
    resolved_match_id = afl_match_id or stored_match_id
    if _lifecycle_rank(stored_status) >= _lifecycle_rank("CONCLUDED"):
        return MatchStatusResolution(stored_status, "canonical_match_database",
                                     resolved_match_id, stored_status)
    if resolved_match_id is None:
        return MatchStatusResolution(stored_status,
                                     "canonical_match_database" if stored_status else None,
                                     None, stored_status)
    try:
        payload = client.get("match_detail", path_parameters={"match_id": resolved_match_id}).data
        detail = _match_detail(payload, resolved_match_id, match_provider_id)
    except AflJsonError as exc:
        return MatchStatusResolution(
            stored_status, "canonical_match_database" if stored_status else None,
            resolved_match_id, stored_status,
            diagnostic=f"Direct match detail unavailable: {type(exc).__name__}",
        )
    direct_status = _text(detail.get("status"))
    direct_provider_id = _text(detail.get("providerId"))
    chosen = stored_status
    resolution = "canonical_match_database" if stored_status else None
    refreshed = False
    if _lifecycle_rank(direct_status) > _lifecycle_rank(stored_status):
        chosen = direct_status
        resolution = "public_match_detail"
        if row is not None:
            _refresh_canonical_match_status(conn, stored_match_id, direct_status)
            refreshed = True
    elif stored_status is None and direct_status is not None:
        chosen, resolution = direct_status, "public_match_detail"
    diagnostic = None
    if direct_provider_id and direct_provider_id != match_provider_id:
        diagnostic = (f"Direct match provider ID {direct_provider_id!r} does not match "
                      f"requested {match_provider_id!r}")
    return MatchStatusResolution(chosen, resolution, resolved_match_id, stored_status,
                                 direct_status, refreshed, diagnostic)
...
def _result_status(source_status: str | None, records: list[Any], one_team: bool) -> PlayerStatsStatus:
    ...
    if folded in {"postgame", "post_game"}:
        return PlayerStatsStatus.POSTGAME
    if folded in {"scheduled", "prematch", "pre_match", "upcoming"}:
        return PlayerStatsStatus.SCHEDULED
    if not records:
...
def _lifecycle_rank(value: str | None) -> int:
    classification = _status_class(value) if value else PlayerStatsStatus.UNKNOWN
    return {
        PlayerStatsStatus.UNKNOWN: -1,
        PlayerStatsStatus.SCHEDULED: 0,
        PlayerStatsStatus.LIVE_PARTIAL: 1,
        PlayerStatsStatus.POSTGAME: 2,
        PlayerStatsStatus.CONCLUDED: 3,
    }.get(classification, -1)


def _forward_status(endpoint_status: str | None, resolved_status: str | None) -> str | None:
    """Choose recognised status monotonically; endpoint data cannot downgrade metadata."""
    if _lifecycle_rank(endpoint_status) >= _lifecycle_rank(resolved_status):
        return endpoint_status or resolved_status
...
def _refresh_canonical_match_status(conn: sqlite3.Connection, match_id: int,
                                    status: str) -> None:
    columns = {row[1] for row in conn.execute("PRAGMA table_info(matches)")}
    observed_at = datetime.now(timezone.utc).isoformat()
    assignments = ["status=?"]
    parameters: list[Any] = [status]
    # scraped_at belongs to the bulk public metadata scrape. This targeted
    # reconciliation is a canonical-row update, so only updated_at advances.
    if "updated_at" in columns:
        assignments.append("updated_at=?")
        parameters.append(observed_at)
    conn.execute(f"UPDATE matches SET {', '.join(assignments)} WHERE match_id=?",
                 (*parameters, match_id))


def _match_detail(payload: Any, match_id: int, match_provider_id: str) -> Mapping[str, Any]:
    matches = payload.get("matches") if isinstance(payload, dict) else None
    if not isinstance(matches, list) or not matches:
        raise AflJsonInvalidResponse(
            f"Direct match detail for {match_id} has no matches collection", endpoint="match_detail"
        )
    candidates = [item for item in matches if isinstance(item, dict)
                  and item.get("id") == match_id]
    if len(candidates) != 1:
        raise AflJsonInvalidResponse(
            f"Direct match detail for {match_id} did not identify exactly one match",
            endpoint="match_detail",
        )
    detail = candidates[0]
    if _text(detail.get("providerId")) != match_provider_id:
        raise AflJsonInvalidResponse(
            f"Direct match detail for {match_id} has an unexpected provider ID",
            endpoint="match_detail",
        )
    return detail
