...
| Public AFL JSON player ID map + CFS season players | `canonical_players`, `player_provider_ids`, `competition_season_players` | `GET /api/v1/players`, `/players/{id}`, `/players/{id}/seasons`, `/seasons/{season_id}/players` | consumed | Backs player search/lookup, the season/team navigation added by Issue #182, and the season-population direction of that same navigation added by Issue #247. `competition_season_players` remains the sole population authority for the last route -- never StatsPro, derived H&A, match appearances, rosters, or the legacy `players` table. |
...
| CFS `matchRosters/round/{id}` (production path, Issue #219) | `cfs_match_rosters`/`cfs_match_roster_selections`/`cfs_match_roster_context` (migration `0024`) | `GET /api/v1/matches/{match_id}/rosters` | exposed/unused | Newly shipped; a team selection, never proof of match participation -- see `docs/api_v1_rosters.md`. A distinct authority from the pre-existing legacy HTML `lineups` model, which is unaffected. |
