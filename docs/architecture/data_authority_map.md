...
| Player statistics | Authenticated CFS JSON | `cfs_player_stats` | HTML `player_stats` is compatibility/diagnostic storage | Opaque `match_provider_id` and `champion_data_player_id`; optional `canonical_player_id`; supplied numeric `afl_match_id` | New reads must not silently use `player_stats`; the current API still exposes that compatibility model. |
