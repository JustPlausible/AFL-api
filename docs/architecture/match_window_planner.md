...
A claimed window's cached `lifecycle` is only used to decide *whether* an attempt is eligible to run at all (Issue #145). Once an attempt proceeds, the worker re-reads current canonical `matches.status` immediately before collection (a single read-only query, not a network call) and passes that fresher value — combined monotonically with the claimed lifecycle — as `canonical_match_status` to the CFS collector, and uses it for cadence/phase selection. This prevents a window whose `lifecycle` has not yet been reconciled from repeatedly resolving and persisting a stale `LIVE` `resolved_match_status`, or staying on live cadence, after the canonical match has already concluded.

`match_windows.reconcile`'s valid-lease branch deliberately leaves an actively-leased window's `collection_phase`, `status`, and `cadence_profile` alone (only `lifecycle` itself is refreshed) so it never mutates state an in-flight attempt owns. The next attempt against that lease is therefore the first point at which those fields can catch up: when the freshly-resolved lifecycle has moved to `POSTGAME`/`CONCLUDED`, `_persist_success` folds `collection_phase='final_confirmation'` and the `post_match_cadence`/`post_match_awaiting_final` cadence decision into that same single write, including when the CFS response itself still classifies as `live_partial` (endpoint still lagging). This is distinguished from a genuinely ambiguous partial response (where the resolved lifecycle itself is unclear) which keeps the separate, independently configurable `partial_cadence`.
