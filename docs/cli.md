...
| Bootstrap one AFL season | `python cli.py --bootstrap-afl-season <SEASON_YEAR>` | Opens the database; upserts stable canonical team metadata, players, provider mappings, authoritative `afl_team_seasons` links, and player-season membership without changing other seasons | Public AFL JSON metadata followed by CFS season players; concise summary by default, full result with `--print-json`; no HTML fallback | Outbound AFL access and CFS auth |
