...
`--afl-match-id`) in the configured canonical database. The provider-ID lookup
automatically supplies the stored numeric AFL match ID; `--afl-match-id` is only
an override/fallback. Unless that row is
already terminal `CONCLUDED`, it then requests the current direct public match
detail at `/afl/v2/matches/{aflMatchId}`. A more advanced lifecycle value from
that detail response replaces the stored canonical status and refreshes
`updated_at`. `scraped_at` continues to mean collection by the bulk public
metadata scraper and is deliberately not changed by targeted reconciliation.
For manual testing, an explicit metadata fallback can
be supplied without claiming it came from CFS:
...
An endpoint status participates in the same recognised, forward-only lifecycle
comparison as resolved metadata; it does not automatically take precedence.
A stale endpoint `LIVE` therefore cannot downgrade reconciled `CONCLUDED`
metadata. Conflicting values retain both provenance fields and produce a
diagnostic. A supplied single/null team array or effective live status is
...
The external resolution order is:

1. read the canonical `matches` row by Champion Data provider ID or AFL ID;
2. keep stored `CONCLUDED` without a network request or downgrade;
3. when an AFL numeric ID is known, request direct public match detail and use
   it only if it advances `SCHEDULED` → `LIVE` → `POSTGAME` → `CONCLUDED`;
4. persist that advancement to the canonical match row;
5. if detail is unavailable, safely retain stored canonical metadata;
6. remain unknown if neither source supplies a recognised lifecycle value.

The season metadata collector uses the public round/list endpoint
`/afl/v2/matches?competitionId=...&compSeasonId=...&roundNumber=...`. Live
verification observed that this bulk response could still say `LIVE` while
`/afl/v2/matches/8207` said `CONCLUDED`; a later repeat returned `CONCLUDED`
from both. The direct resource is therefore the finalisation authority rather
than assuming the list view is synchronised immediately.
...
For match `8207`, repeat the status and collection verification with:

```bash
curl -fsSL https://aflapi.afl.com.au/afl/v2/matches/8207 | jq '.matches[0] | {id, providerId, status}'
python cli.py --collect-match-player-stats CD_M20260142007 --afl-match-id 8207 --print-json
sqlite3 "$DB_PATH" "SELECT match_id, match_provider_id, status FROM matches WHERE match_id=8207;"
sqlite3 "$DB_PATH" "SELECT COUNT(*), MIN(snapshot_authority), MAX(snapshot_authority) FROM cfs_player_stats WHERE match_provider_id='CD_M20260142007';"
```
