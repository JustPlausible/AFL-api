...
`GET /api/v1/players/{canonical_player_id}/movements` returns resolved supplemental observations, including the originating team, original label/detail, AFL article link, snapshot source URL, archive timestamp, explicit snapshot timestamp, and observation time. Multiple live observations may be returned as distinct evidence rows, ordered newest snapshot first; the API does not destructively collapse historical evidence. A known player with no movement history returns an empty `movements` list; an unknown canonical ID returns the normal 404 application error.

`reconcile_player_movements` is a read-only operator building block for previous/current membership comparison. Both adjacent seasons must resolve uniquely in the canonical AFL competition, so same-year AFLW or other competition memberships cannot duplicate or alter transitions. Reconciliation raises a clear error rather than inferring a transition when either AFL season is missing or ambiguous. It reports `same_club`, `changed_club`, `absent_from_next_population`, or `unresolved`; a movement may explain a factual transition but never causes or guesses one.
