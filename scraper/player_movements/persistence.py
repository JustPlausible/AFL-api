 ...
 def persist(self,resolved,document,*,movement_season_year,counts_by_type):
  ...
  try:
   for r in resolved.records:
    s=r.source; key=(movement_season_year,document.source_url,document.source_archived_at,s.team_name,s.player_name,s.source_label)
    old=self.conn.execute('SELECT id,canonical_player_id,from_team_id,movement_type,source_detail,article_url,resolution_status,resolution_reason FROM player_movement_observations WHERE movement_season_year=? AND source_url=? AND source_archived_at IS ? AND source_team_name=? AND source_player_name=? AND source_label=?',key).fetchone()
    values=(r.canonical_player_id,r.from_team_id,s.movement_type,s.source_detail,s.article_url,r.status,r.reason)
    if old is None:
     self.conn.execute('''INSERT INTO player_movement_observations(canonical_player_id,movement_season_year,from_team_id,movement_type,source_label,source_detail,source_player_name,source_team_name,article_url,source_url,source_archived_at,observed_at,resolution_status,resolution_reason,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(r.canonical_player_id,movement_season_year,r.from_team_id,s.movement_type,s.source_label,s.source_detail,s.player_name,s.team_name,s.article_url,document.source_url,document.source_archived_at,document.observed_at,r.status,r.reason,document.observed_at,document.observed_at)); inserted+=1
