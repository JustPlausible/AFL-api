    ...
    for match in match_blocks:
        ...
        if content_section:
            for row in content_section.select("div.team-lineups__row"):
                ...
                log.info(f"📌 {status_label} – {len(players_home)} home, {len(players_away)} away")
                ...
                for tag in players_home:
                    ...
                    log.info(f"  🔵 {player['first_name']} {player['surname']} ({status_label})")
        ...
        for row in match.select("div.team-lineups__positions-row"):
            ...
            for container in containers:
                ...
                log.info(f"🎯 {team_name} - {position_group}: {len(player_tags)} players")
                ...
                for tag in player_tags:
                    ...
                    all_players.append(player)
                    log.info(f"  ✅ {player['first_name']} {player['surname']} ({player['position_group']}, AFL ID: {player['afl_id']})")
                            ...
                            if outs_match:
                                ...
                                for shortname in names:
                                    ...
                                    if surname in named_players_by_match.get(match_id, {}):
                                        ...
                                        log.info(f"🟠 Late OUT flagged: {player['first_name']} {player['surname']} ({match_id})")
                                    else:
                                        log.warning(f"⚠️ Late OUT '{shortname}' not matched to known line-up in match {match_id}")
...
if __name__ == "__main__":
    ...
    log.info(f"🧾 Sample player: {players[0] if players else 'No players found.'}")
