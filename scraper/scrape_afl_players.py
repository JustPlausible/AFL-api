...
def scroll_leaderboard_to_load_images(page, scroll_step=1000, delay_ms=200, max_scrolls=30):
    ...
    for i in range(max_scrolls):
        ...
        log.debug(f"↕️ Scrolled leaderboard container [{i+1}/{max_scrolls}]")
...
def force_load_all_player_images(page):
    ...
    for img in image_elements:
        try:
            ...
            log.warning(f"⚠️ Failed scrollIntoView: {e}")
...
def load_all_stats_rows(page):
    ...
    for i in range(max_clicks):
        ...
        if button and button.is_visible():
            ...
            log.debug("✅ All rows loaded or button hidden.")
            break
...
def parse_row(row):
    try:
        ...
        log.warning(f"⚠️ Failed to parse a row: {e}")
        return None
...
def scrape_afl_stats_leaderboard():
    with sync_playwright() as p:
        ...
        try:
            ...
            log.info(f"✅ SUCCESS: 📄 Saved {len(players)} players to {OUTPUT_FILE}")
        finally:
            browser.close()
