...
def test_cli_persists_stats_resolves_concluded_status_and_is_idempotent(
    ...
    assert first == {
        ...
        "canonical_status_refreshed": False,
...
def test_cli_refreshes_stored_postgame_and_persists_concluded_snapshot(
    cli_database, monkeypatch, capsys,
):
    conn = sqlite3.connect(cli_database)
    conn.execute("UPDATE matches SET match_id=8207, match_provider_id='CD_M20260142007', status='POSTGAME'")
    conn.commit()
    conn.close()
    payload = concluded_payload()
    payload.pop("status", None)
    FakeClient.payload = payload
    FakeClient.detail_payload = json.loads(
        (FIXTURES / "match_detail_concluded.json").read_text()
    )
    FakeClient.unavailable = False
    monkeypatch.setattr(sys, "argv", [
    ...
    assert output["status"] == "concluded"
    assert output["resolved_match_status"] == "CONCLUDED"
    assert output["status_resolution"] == "public_match_detail"
    assert output["canonical_status_refreshed"] is True
    assert output["rows_written"] == 2
    conn = sqlite3.connect(cli_database)
    assert conn.execute("SELECT status FROM matches WHERE match_id=8207").fetchone() == ("CONCLUDED",)
