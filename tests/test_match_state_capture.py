...
def test_settings_disabled_when_globally_disabled_even_if_profile_named(monkeypatch):
    import config
    monkeypatch.setattr(config, "AFL_DIAGNOSTICS_ENABLED", False, raising=False)
    monkeypatch.setattr(config, "AFL_DIAGNOSTIC_PROFILES", ("match_clock",), raising=False)
    assert MatchStateCaptureSettings.from_config().enabled is False


def test_settings_disabled_when_globally_enabled_but_profile_not_named(monkeypatch):
    import config
    monkeypatch.setattr(config, "AFL_DIAGNOSTICS_ENABLED", True, raising=False)
    monkeypatch.setattr(config, "AFL_DIAGNOSTIC_PROFILES", ("some_other_profile",), raising=False)
    assert MatchStateCaptureSettings.from_config().enabled is False


# Registration through the diagnostics framework (scheduler wiring, job IDs,
# and generic enable/disable behaviour) is covered end-to-end in
# tests/test_diagnostics_framework.py using this module's MatchClockProfile.
